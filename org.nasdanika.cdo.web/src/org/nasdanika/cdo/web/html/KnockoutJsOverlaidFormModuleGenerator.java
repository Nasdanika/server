...
public class KnockoutJsOverlaidFormModuleGenerator<C extends org.nasdanika.core.Context, T> extends org.nasdanika.core.AbstractCommand<C,T,String> {
  ...
  protected final String TEXT_6 = NL + "\t\t\t\t}" + NL + "\t\t\t}.bind(this.model)," + NL + "\t\t\tfunction(reason) {" + NL + "\t\t\t\toverlay.style.display = \"none\";\t\t" + NL + "\t\t\t\tif (reason.targetInvocationError) {" + NL + "\t\t\t\t\tthis.validationResults['$this'] = reason.targetInvocationError;" + NL + "\t\t\t\t} else {" + NL + "\t\t\t\t\tthis.validationResults['$this'] = JSON.strinfigy(reason);\t\t\t\t\t" + NL + "\t\t\t\t}" + NL + "\t\t\t}.bind(this.model)).done();" + NL + "\t\t}," + NL + "\t\t";
