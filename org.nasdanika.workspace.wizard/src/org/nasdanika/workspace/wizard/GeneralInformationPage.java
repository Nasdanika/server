...
public class GeneralInformationPage extends WizardPage {

	// initial value stores
	private String initialGroupIdFieldValue;
...
//	};
	...
	private String initialVersionFieldValue = "1.0.0";
	 ...
	 */
	public GeneralInformationPage(IStructuredSelection selection, String pageName) {
		super(pageName);
	...
	public void createControl(Composite parent) {
		...
		if (initialGroupIdFieldValue != null) {
			locationArea.updateProjectName(initialGroupIdFieldValue);
		}
	...
	private final void createProjectNameGroup(Composite parent) {
		...
		Composite groupGroup = new Composite(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.numColumns = 2;
		groupGroup.setLayout(layout);
		groupGroup.setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

		createNameLabelAndText(parent, groupGroup);
		createGroupLabelAndText(parent, groupGroup);
		createVersionLabelAndText(parent, groupGroup);
		
		// Set the initial value first before listener
		// to avoid handling an event during the creation.
		if (initialVersionFieldValue != null) {
			versionField.setText(initialVersionFieldValue);
		}
//		versionField.addListener(SWT.Modify, nameModifyListener);
		BidiUtils.applyBidiProcessing(versionField, BidiUtils.BTD_DEFAULT);
	}

	private void createVersionLabelAndText(Composite parent, Composite groupGroup) {
		// new version label
		Label versionLabel = new Label(groupGroup, SWT.NONE);
		versionLabel.setText("Version:");
		versionLabel.setFont(parent.getFont());

		// new version name entry field
		versionField = new Text(groupGroup, SWT.BORDER);
		versionField.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				GeneralInformationPage.this.setPageComplete(
						groupIdField.getText().trim().length()>0
						&& nameField.getText().trim().length()>0
						&& versionField.getText().trim().length()>0);
			}
		});
		GridData versionFieldData = new GridData(GridData.FILL_HORIZONTAL);
		versionFieldData.widthHint = SIZING_TEXT_FIELD_WIDTH;
		versionField.setLayoutData(versionFieldData);
		versionField.setFont(parent.getFont());
	}

	private void createGroupLabelAndText(Composite parent, Composite groupGroup) {
		// new group label
		Label groupIdLabel = new Label(groupGroup, SWT.NONE);
		groupIdLabel.setText("Group Id:");
		groupIdLabel.setFont(parent.getFont());

		// new group name entry field
		groupIdField = new Text(groupGroup, SWT.BORDER);
...
//				if (groupIdField.getText().trim().length()==0) {
//					setErrorMessage("Group ID is blank");
//				}
			}
		});
		GridData data = new GridData(GridData.FILL_HORIZONTAL);
		data.widthHint = SIZING_TEXT_FIELD_WIDTH;
		groupIdField.setLayoutData(data);
		groupIdField.setFont(parent.getFont());
	}


	private void createNameLabelAndText(Composite parent, Composite groupGroup) {
		// new name label
		Label nameLabel = new Label(groupGroup, SWT.NONE);
		nameLabel.setText("Name:");
		nameLabel.setFont(parent.getFont());

		// new name entry field
		nameField = new Text(groupGroup, SWT.BORDER);
...
//			return initialGroupIdFieldValue;
//		}
//
...
//	}
	...
	public void setInitialProjectName(String name) {
		if (name == null) {
			initialGroupIdFieldValue = null;
		} else {
			initialGroupIdFieldValue = name.trim();
