...
public class CDOObjectModuleLazyObjectFacadeDefinitionGenerator implements org.nasdanika.cdo.web.routes.Generator {
  ...
  protected final String TEXT_7 = NL + "\t\t\t\t\t\t\tvar promises = [];" + NL + "\t\t\t\t\t\t\tfor (ref in data) {" + NL + "\t\t\t\t\t\t\t\tpromises.push(Q.Promise(function(idx, resolve, reject, notify) {" + NL + "\t\t\t\t\t\t\t\t\trequire([data[idx]+\".js\"], function(mod) {" + NL + "\t\t\t\t\t\t\t\t\t\tresolve(mod);" + NL + "\t\t\t\t\t\t\t\t\t});\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t\t\t}.bind(this, ref)));" + NL + "\t\t\t\t\t\t\t}" + NL + "\t\t\t\t\t\t\tQ.all(promises).then(function(res) { vResolve(res); });" + NL + "\t\t\t\t\t\t";
  protected final String TEXT_8 = NL + "\t\t\t\t\t\t\trequire([data+\".js\"], function(mod) {" + NL + "\t\t\t\t\t\t\t\tvResolve(mod);" + NL + "\t\t\t\t\t\t\t});\t\t\t\t" + NL + "\t\t\t\t\t\t";
