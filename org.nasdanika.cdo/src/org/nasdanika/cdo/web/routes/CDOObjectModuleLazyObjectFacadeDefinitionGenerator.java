...
public class CDOObjectModuleLazyObjectFacadeDefinitionGenerator implements org.nasdanika.cdo.web.routes.Generator {
  ...
  protected final String TEXT_8 = NL + "\t\t\t\t\t\t\tvar promises = [];" + NL + "\t\t\t\t\t\t\tfor (var ref in refData) {" + NL + "\t\t\t\t\t\t\t\tpromises.push(Q.Promise(function(idx, resolve, reject, notify) {" + NL + "\t\t\t\t\t\t\t\t\trequire([refData[idx]+\".js\"], function(mod) {" + NL + "\t\t\t\t\t\t\t\t\t\tresolve(mod);" + NL + "\t\t\t\t\t\t\t\t\t});\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t" + NL + "\t\t\t\t\t\t\t\t}.bind(this, ref)));" + NL + "\t\t\t\t\t\t\t}" + NL + "\t\t\t\t\t\t\tQ.all(promises).then(function(res) { vResolve(res); });" + NL + "\t\t\t\t\t\t";
