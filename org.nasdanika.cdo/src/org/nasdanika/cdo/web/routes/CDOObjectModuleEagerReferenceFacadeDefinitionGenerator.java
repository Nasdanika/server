...
public class CDOObjectModuleEagerReferenceFacadeDefinitionGenerator implements org.nasdanika.cdo.web.routes.Generator {
  ...
  protected final String TEXT_4 = NL + "\t\t\tdataEntry.value = [];" + NL + "\t\t\tfor (ref in dataEntry.initialValue) {" + NL + "\t\t\t\tdataEntry.value.push(function(modName) {" + NL + "\t\t\t\t\treturn Q.Promise(function(resolve, reject, notify) {" + NL + "\t\t\t\t\t\trequire([modName], function(mod) {" + NL + "\t\t\t\t\t\t\tresolve(mod);" + NL + "\t\t\t\t\t\t});\t\t\t\t\t" + NL + "\t\t\t\t\t});" + NL + "\t\t\t\t}.bind(this, dataEntry.initialValue[ref]+\".js\"));\t\t\t\t" + NL + "\t\t\t}" + NL + "\t\t";
  protected final String TEXT_5 = NL + "\t\t\tdataEntry.value = Q.Promise(function(resolve, reject, nofity) {" + NL + "\t\t\t\trequire([dataEntry.initialValue+\".js\"], function(mod) {" + NL + "\t\t\t\t\tresolve(mod);" + NL + "\t\t\t\t\t//dataEntry.value = mod; " + NL + "\t\t\t\t});\t\t\t\t\t\t\t" + NL + "\t\t\t});" + NL + "\t\t";
